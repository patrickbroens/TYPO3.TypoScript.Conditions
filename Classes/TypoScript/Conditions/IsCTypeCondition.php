 ...
 * Is CType condition
 */
class IsCTypeCondition extends AbstractCondition
{
    /**
     * Condition used to check if a content element is a certain content element
        ...
        if (0 === count($conditionParameters)) {
            ...
            if (
                ...
                if (is_array($getUid)) {
                    ...
                    $result = $cType === $this->getCTypeFromDatabase($uid);
                }
            }
        }
    ...
    /**
     * Get the CType from a content element in the database
     *
     * @param int $contentElementUid The content element uid
     * @return int The CType
     */
    protected function getCTypeFromDatabase($contentElementUid)
    {
        $queryResult = $this->getDatabaseConnection()->sql_query(
            'SELECT CType FROM tt_content WHERE uid =' . $contentElementUid
        );
        ...
        return (string)$contentElement['CType'];
    }
